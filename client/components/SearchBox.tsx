    ...
    <div ref={searchRef} className={`relative ${className}`}>
      <div className="relative">
        <input
          ...
          className="w-full bg-vintage-black/50 border border-vintage-gold/30 rounded-lg px-4 py-2 pl-10 pr-10 text-vintage-cream placeholder-vintage-cream/50 focus:border-vintage-gold focus:outline-none font-vintage-body"
        />
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-vintage-black/95 backdrop-blur-sm border border-vintage-gold/30 rounded-lg shadow-xl z-50 max-h-96 overflow-y-auto vintage-scrollbar">
          <div className="p-2">
            <div className="flex items-center justify-between px-3 py-2 border-b border-vintage-gold/20">
              <span className="text-sm text-vintage-gold font-vintage-serif font-semibold">
                {searchResults.length} resultado{searchResults.length !== 1 ? 's' : ''} encontrado{searchResults.length !== 1 ? 's' : ''}
              </span>
              <Link
                to={`/filmes?q=${encodeURIComponent(searchQuery)}`}
                onClick={() => hideResults()}
                className="text-xs text-vintage-cream/70 hover:text-vintage-gold transition-colors font-vintage-body"
              >
                Ver todos →
              </Link>
            </div>
              ...
              {searchResults.map(({ filme, matchType }) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 
                      className="font-vintage-serif font-semibold text-vintage-cream group-hover:text-vintage-gold transition-colors line-clamp-1"
                      dangerouslySetInnerHTML={{ 
                        __html: highlightMatch(filme.nomePortugues, searchQuery) 
                      }}
                    />
                    <p 
                      className="text-sm text-vintage-cream/70 italic line-clamp-1 font-vintage-body"
                      dangerouslySetInnerHTML={{ 
                    ...
                    <div className="flex items-center space-x-3 mt-1">
                      ...
                      {filme.avaliacoes && (
                        <div className="flex items-center space-x-1 text-xs text-vintage-cream/60">
                          ...
                          <span className="font-vintage-body">
                            {Math.round(
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <span className="text-xs bg-vintage-gold/20 text-vintage-gold px-2 py-0.5 rounded font-vintage-body">
                        {getMatchTypeLabel(matchType)}
                      </span>
                      {filme.categoria.slice(0, 2).map((cat, index) => (
                        <span
                          key={index}
                          className="text-xs bg-vintage-black/30 text-vintage-cream/60 px-2 py-0.5 rounded font-vintage-body"
                        >
                          {cat}
                        </span>
                      ))}
      ...
      {showResults && showResultsContext && searchQuery && searchResults.length === 0 && !isSearching && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-vintage-black/95 backdrop-blur-sm border border-vintage-gold/30 rounded-lg shadow-xl z-50">
          <div className="p-6 text-center">
            ...
            <h4 className="text-lg font-vintage-serif font-semibold text-vintage-gold mb-2">
